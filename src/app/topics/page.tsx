...
function TopicsContent() {
  ...
  React.useEffect(() => {
    if (!user) return;
    (async () => {
      const [nextCategories, personalVideos] = await Promise.all([
        listCategories(), listAllPersonalVideos(user.uid),
      ]);
  ...
  return <AppShell><div className="mx-auto max-w-5xl space-y-6">
    <div><h1 className="flex items-center gap-2 font-display text-2xl font-semibold"><BookOpen className="h-5 w-5 text-accent" /> Browse by Topic</h1><p className="mt-1 text-sm text-muted-foreground">Explore your personal content by topic.</p></div>
